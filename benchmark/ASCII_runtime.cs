...
namespace Competition
{
    ...
    // taken from: https://source.dot.net/#System.Private.CoreLib/src/libraries/System.Private.CoreLib/src/System/Text/Ascii.Utility.Helpers.cs,2a01aa9cd5e8b40f
    public static partial class Ascii
    {

        //Copy pasted from: https://source.dot.net/#System.Private.CoreLib/src/libraries/System.Private.CoreLib/src/System/Runtime/Intrinsics/Vector128.cs,81388cb25e9aede5
        /// <summary>
    ...
    public static partial class Ascii
    {
        ...
        private static unsafe nuint GetIndexOfFirstNonAsciiByte_Vector(byte* pBuffer, nuint bufferLength)
        {
            ...
            if (Vector512.IsHardwareAccelerated && bufferLength >= 2 * (uint)Vector512<byte>.Count)
            {
                if (Vector512.Load(pBuffer).ExtractMostSignificantBits() == 0)
                {
                    ...
                    byte* pFinalVectorReadPos = pBuffer + bufferLength - 64;
                    pBuffer = (byte*)(((nuint)pBuffer + 64) & ~(nuint)(64 - 1));

#if DEBUG
                    long numBytesRead = pBuffer - pOriginalBuffer;
                    Debug.Assert(0 < numBytesRead && numBytesRead <= 64, "We should've made forward progress of at least one byte.");
                    ...
                    do
                    {
                        Debug.Assert((nuint)pBuffer % 64 == 0, "Vector read should be aligned.");
            ...
            else if (Vector256.IsHardwareAccelerated && bufferLength >= 2 * (uint)Vector256<byte>.Count)
            {
                if (Vector256.Load(pBuffer).ExtractMostSignificantBits() == 0)
                {
                    ...
                    byte* pFinalVectorReadPos = pBuffer + bufferLength - 32;
                    pBuffer = (byte*)(((nuint)pBuffer + 32) & ~(nuint)(32 - 1));

#if DEBUG
                    long numBytesRead = pBuffer - pOriginalBuffer;
                    Debug.Assert(0 < numBytesRead && numBytesRead <= 32, "We should've made forward progress of at least one byte.");
                    Debug.Assert((nuint)numBytesRead <= bufferLength, "We shouldn't have read past the end of the input buffer.");
#endif
                    ...
                    do
                    {
                        Debug.Assert((nuint)pBuffer % 32 == 0, "Vector read should be aligned.");
            ...
            else if (Vector128.IsHardwareAccelerated && bufferLength >= 2 * (uint)Vector128<byte>.Count)
            {
                if (!VectorContainsNonAsciiChar(Vector128.Load(pBuffer)))
                {
                    ...
                    byte* pFinalVectorReadPos = pBuffer + bufferLength - 16;
                    pBuffer = (byte*)(((nuint)pBuffer + 16) & ~(nuint)(16 - 1));

#if DEBUG
                    long numBytesRead = pBuffer - pOriginalBuffer;
                    Debug.Assert(0 < numBytesRead && numBytesRead <= 16, "We should've made forward progress of at least one byte.");
                    Debug.Assert((nuint)numBytesRead <= bufferLength, "We shouldn't have read past the end of the input buffer.");
#endif
                    ...
                    do
                    {
                        Debug.Assert((nuint)pBuffer % 16 == 0, "Vector read should be aligned.");
        ...
        private static unsafe nuint GetIndexOfFirstNonAsciiChar_Vector(char* pBuffer, nuint bufferLength /* in chars */)
        {
            ...
            {
                const uint SizeOfVector512InChars = 64 / sizeof(ushort);
                ...
                if (!VectorContainsNonAsciiChar(Vector512.Load((ushort*)pBuffer)))
                {
                    ...
                    pBuffer = (char*)(((nuint)pBuffer + 64) & ~(nuint)(64 - 1));
            ...
            if (Vector256.IsHardwareAccelerated && bufferLength >= 2 * (uint)Vector256<ushort>.Count)
            {
                const uint SizeOfVector256InChars = 32 / sizeof(ushort);
                ...
                if (!VectorContainsNonAsciiChar(Vector256.Load((ushort*)pBuffer)))
                {
                    ...
                    pBuffer = (char*)(((nuint)pBuffer + 32) & ~(nuint)(32 - 1));
            ...
            {
                const uint SizeOfVector128InChars = 16 / sizeof(ushort); // JIT will make this a const
                ...
                if (!VectorContainsNonAsciiChar(Vector128.Load((ushort*)pBuffer)))
                {
                    ...
                    pBuffer = (char*)(((nuint)pBuffer + 16) & ~(nuint)(16 - 1));
        ...
        private static unsafe nuint GetIndexOfFirstNonAsciiChar_Intrinsified(char* pBuffer, nuint bufferLength /* in chars */)
        {
            ...
            uint SizeOfVector128InChars = 16 / sizeof(char);
            ...
            if (bufferLength >= 2 * 16)
            {
                char* pFinalVectorReadPos = (char*)((nuint)pBuffer + bufferLength - 2 * 16);
        ...
        DoFinalUnalignedVectorRead:
            ...
            if (((byte)bufferLength & (16 - 1)) != 0)
            {
                // Perform an unaligned read of the last vector.
                // We need to adjust the pointer because we're re-reading data.

                pBuffer = (char*)((byte*)pBuffer + (bufferLength & (16 - 1)) - 16);
                firstVector = Vector128.LoadUnsafe(ref *(ushort*)pBuffer);
                if (VectorContainsNonAsciiChar(firstVector))
                {
                    goto FoundNonAsciiDataInFirstVector;
        ...
        private static unsafe nuint NarrowUtf16ToAscii_Intrinsified_256(char* pUtf16Buffer, byte* pAsciiBuffer, nuint elementCount)
        {
            ...
            const nuint MaskOfAllBitsInVector256 = (nuint)(32 - 1);
            ...
            Debug.Assert(BitConverter.IsLittleEndian, "This implementation assumes little-endian.");
            Debug.Assert(elementCount >= 2 * 32);
            ...
            nuint currentOffsetInElements = 32 / 2; // we processed 16 elements so far
            ...
            currentOffsetInElements = 32 - ((nuint)pAsciiBuffer & MaskOfAllBitsInVector256);

            Debug.Assert(0 < currentOffsetInElements && currentOffsetInElements <= 32, "We wrote at least 1 byte but no more than a whole vector.");
            Debug.Assert(currentOffsetInElements <= elementCount, "Shouldn't have overrun the destination buffer.");
            Debug.Assert(elementCount - currentOffsetInElements >= 32, "We should be able to run at least one whole vector.");

            nuint finalOffsetWhereCanRunLoop = elementCount - 32;
            do
            {
                // In a loop, perform two unaligned reads, narrow to a single vector, then aligned write one vector.

                utf16VectorFirst = Vector256.LoadUnsafe(ref utf16Buffer, currentOffsetInElements);
                Vector256<ushort> utf16VectorSecond = Vector256.LoadUnsafe(ref utf16Buffer, currentOffsetInElements + 32 / sizeof(short));
                ...
                Debug.Assert(((nuint)pAsciiBuffer + currentOffsetInElements) % 32 == 0, "Write should be aligned.");
                asciiVector = Vector256.Narrow(utf16VectorFirst, utf16VectorSecond);
                asciiVector.StoreUnsafe(ref asciiBuffer, currentOffsetInElements);

                currentOffsetInElements += 32;
            } while (currentOffsetInElements <= finalOffsetWhereCanRunLoop);
        ...
        FoundNonAsciiDataInLoop:
            ...
            Debug.Assert(((nuint)pAsciiBuffer + currentOffsetInElements) % 16 == 0, "Destination should be 128-bit-aligned.");
            asciiVector = Vector256.Narrow(utf16VectorFirst, utf16VectorFirst);
            asciiVector.GetLower().StoreUnsafe(ref asciiBuffer, currentOffsetInElements);
            currentOffsetInElements += 32 / 2;
        ...
        private static unsafe nuint NarrowUtf16ToAscii_Intrinsified_512(char* pUtf16Buffer, byte* pAsciiBuffer, nuint elementCount)
        {
            ...
            const nuint MaskOfAllBitsInVector512 = (nuint)(64 - 1);
            ...
            Debug.Assert(BitConverter.IsLittleEndian, "This implementation assumes little-endian.");
            Debug.Assert(elementCount >= 2 * 64);
            ...
            nuint currentOffsetInElements = 64 / 2; // we processed 32 elements so far
            ...
            currentOffsetInElements = 64 - ((nuint)pAsciiBuffer & MaskOfAllBitsInVector512);

            Debug.Assert(0 < currentOffsetInElements && currentOffsetInElements <= 64, "We wrote at least 1 byte but no more than a whole vector.");
            Debug.Assert(currentOffsetInElements <= elementCount, "Shouldn't have overrun the destination buffer.");
            Debug.Assert(elementCount - currentOffsetInElements >= 64, "We should be able to run at least one whole vector.");

            nuint finalOffsetWhereCanRunLoop = elementCount - 64;
            do
            {
                // In a loop, perform two unaligned reads, narrow to a single vector, then aligned write one vector.

                utf16VectorFirst = Vector512.LoadUnsafe(ref utf16Buffer, currentOffsetInElements);
                Vector512<ushort> utf16VectorSecond = Vector512.LoadUnsafe(ref utf16Buffer, currentOffsetInElements + 64 / sizeof(short));
                ...
                Debug.Assert(((nuint)pAsciiBuffer + currentOffsetInElements) % 64 == 0, "Write should be aligned.");
                asciiVector = Vector512.Narrow(utf16VectorFirst, utf16VectorSecond);
                asciiVector.StoreUnsafe(ref asciiBuffer, currentOffsetInElements);

                currentOffsetInElements += 64;
            } while (currentOffsetInElements <= finalOffsetWhereCanRunLoop);
        ...
        FoundNonAsciiDataInLoop:
            ...
            Debug.Assert(((nuint)pAsciiBuffer + currentOffsetInElements) % 32 == 0, "Destination should be 256-bit-aligned.");
            asciiVector = Vector512.Narrow(utf16VectorFirst, utf16VectorFirst);
            asciiVector.GetLower().StoreUnsafe(ref asciiBuffer, currentOffsetInElements);
            currentOffsetInElements += 64 / 2;
